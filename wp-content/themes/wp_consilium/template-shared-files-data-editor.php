 ...
 *
 * @package WP CONSILIUM
 * @since WP CONSILIUM 1.0
 */
...
error_log("outerKey " . print_r($outerKey, true));			
    error_log(">>> outerKey = " . var_export($outerKey, true));

    if (!in_array($outerKey, ["keys", "headrow", "headrowKat", "headRowSingleFields", "args", "total"], true)) {
        if (array_key_exists($outerKey, $data)) {
            $dataRowArray = $data[$outerKey];
            error_log(">>> dataRowArray (" . gettype($dataRowArray) . "): " . print_r($dataRowArray, true));
        } else {
            error_log("!!! Key {$outerKey} nicht in \$data gefunden!");
        }
    } else {
        error_log("--- übersprungen: {$outerKey}");
    }
		...
		if ($outerKey !== "keys" && $outerKey !== "headrow" && 
			...
			error_log("IN IF!!!! outerKey " . print_r($outerKey, true));
			$dataRowArray = $data[$outerKey];
			...
			foreach ($dataArrayKeys as $dataKey) {
				$element = null;
				error_log("dataKey " . print_r($dataKey, true));
				if (is_array($dataKey)) {
					$firstKey = array_key_first($dataKey);
					$secondKey = $dataKey[$firstKey];
					$element = $dataRowArray[$firstKey][$secondKey];
error_log("is_array first, second element " . print_r($firstKey, true) . ", " . print_r($secondKey, true) . ", " . print_r($element, true) . ", " . print_r($onlyModifySingleField, true));	
					if ($firstKey == "custom_field" && $nurKategorienAnzeigen == false) {
error_log("is_array first, second element IN IF CF onlyModifySingleField " . print_r($onlyModifySingleField, true));
if(str_starts_with($onlyModifySingleField, "file_upload_custom_field_"))
	error_log("is_array first, second element IN IF CF STARTS WITH " . print_r($onlyModifySingleField, true));
if(str_ends_with($onlyModifySingleField, $secondKey))
	error_log("is_array first, second element IN IF CF ENDS WITH " . print_r($onlyModifySingleField, true));
						if(($onlyModifySingleField === null || $onlyModifySingleField === "notselected") ||
							($onlyModifySingleField !== null && 
							str_starts_with($onlyModifySingleField, "file_upload_custom_field_") &&
							str_ends_with($onlyModifySingleField, (string)$secondKey))) {
error_log("is_array first, second element ADD CUSTOM FIELD ");
...
error_log("singleLineResult " . print_r($singleLineResult, true));								
						break;
					}
					else{
...
error_log("description " . print_r($element, true));	
							if(($onlyModifySingleField == null || $onlyModifySingleField == "notselected") ||
							($onlyModifySingleField != null && 
...
error_log("tags " . print_r($element, true));								
							if(($onlyModifySingleField == null || $onlyModifySingleField == "notselected") ||
							($onlyModifySingleField != null && 
  ...
  document.getElementById("cancelReloadPage").addEventListener("click", () => {
	...
	function onExcelExportclick() {
		...
		if (fileName != null && fileName.length > 0) {
			...
			var form = document.getElementById('the-redirect-form');
				console.log("onExcelExportclick", document.getElementById("excelImportFilename").value,
		document.getElementById("doExcelExport").value, form)
			  // --- Event manuell auslösen ---
        	form.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }));
        	// Danach optional wirklich absenden:
        	form.submit();
		} else {
